...
extern "C" {
	...
	// parameter block (q0, q1, np[N]; the layout of mcl::Modp2, see g_modp2)
...
// the parameter block of modp3 (q0, q1, np[N]) computed by mcl::Modp2::init
...
void initModp2()
{
	if (!g_modp2.init(Fp::getOp().mp)) {
		fprintf(stderr, "Modp2::init failed\n");
		exit(1);
	}
}
...
// reference: z[N] = x mod p by mcl's Modp (mpz Barrett), plain representation
...
int main(int argc, char *argv[]) {
	...
	if (ss.empty() || ss.find("modp2") != ss.end()) {
		// 512-bit x mod p; base = mcl's mpz Barrett (Modp). The latency chain
